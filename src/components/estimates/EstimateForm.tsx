...
export function EstimateForm({
  ...
  const action = estimateId
    ? updateEstimateAction.bind(null, estimateId)
    : createEstimateAction;
  const [state, formAction, pending] = useActionState<EstimateFormState, FormData>(
    action,
    undefined
  );
  ...
  const [rows, setRows] = useState<Row[]>(
    defaultValues.items.length > 0 ? defaultValues.items.map((it) => newRow(it)) : [newRow()]
  );
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4">
      ...
      <FormBox fullWidth={fullWidth} gap="gap-4">
          {customers.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              <Select id="customerPick" value={customerId} onChange={(e) => onPickCustomer(e.target.value)}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="dueDate">{t.expiryDate}</Label>
      ...
      <div className="flex flex-col gap-2">
        ...
        <Textarea id="notes" name="notes" rows={3} defaultValue={defaultValues.notes} onChange={markDirty} />
      </div>
