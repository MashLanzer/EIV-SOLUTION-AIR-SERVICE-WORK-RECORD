...
// The Estimates list, extracted so it can render inside the combined Billing
// page. `basePath` is where the search/filter links post back (the page carries
// the ?view=estimates tab); everything else (new/detail/export routes) stays on
// the canonical /admin/estimates paths. Self-guards like the standalone page.
export async function EstimatesPanel({
  q,
  status: rawStatus,
  basePath,
}: {
  ...
  // Filter/search links stay on the combined Billing page, carrying the tab.
  const chipHref = (next?: EstimateStatus | "expired") => {
    const p = new URLSearchParams();
    p.set("view", "estimates");
    if (query) p.set("q", q!.trim());
    if (next) p.set("status", next);
    return `${basePath}?${p.toString()}`;
  };
  const clearHref = `${basePath}?view=estimates`;
