...
// The Invoices list, extracted so it can render inside the combined Billing
// page under the ?view=invoices tab. Filter/search links post back to
// `basePath`; new/detail/export routes stay on /admin/invoices. Self-guards.
export async function InvoicesPanel({
  ...
  const invoicePeeks = filtered.map((i) => ({
    id: i.id,
    numberLabel: formatInvoiceNumber(i.number),
    status: i.status,
    customerName: i.customerName,
    issuedLabel: dateFmt.format(i.issueDate),
    dueLabel: i.dueDate ? dateFmt.format(i.dueDate) : null,
    overdue: i.overdue,
  ...
  const clearHref = `${basePath}?view=invoices`;
    ...
    <>
      ...
      {filtered.length === 0 ? (
        ...
        <>
          ...
          <InvoiceCards invoices={invoicePeeks} />
        </>
      )}
    </>
  );
